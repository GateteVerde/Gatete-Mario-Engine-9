...
if (freeze == false) {
	...
	if (effect == 2) {
		...
		//Generate effect
		with (instance_create_depth(start_x, (round(y + bbox_bottom) / 2) + random_range(-24, 24) - 2, -6, obj_smoke_fade)) {
	...
	//Otherwise, create it on the rear
		...
		//Generate effect
		with (instance_create_depth(point_x, (round(y+bbox_bottom) / 2 - 2), depth + 1, obj_smoke_fade)) {
