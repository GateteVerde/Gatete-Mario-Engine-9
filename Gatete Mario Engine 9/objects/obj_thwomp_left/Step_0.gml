...
#region LOGIC
	...
	&& (collision_rectangle(bbox_left-128, bbox_top, bbox_left, bbox_bottom, obj_mario, 0, 0)) {
	...
	//Otherwise if falling, land on ground
		...
		var mysolid = collision_rectangle(bbox_left-1, bbox_top, bbox_left, bbox_bottom, obj_solid, 1, 0);
		var mylimit = collision_rectangle(bbox_left-1, bbox_top, bbox_left, bbox_bottom, obj_endmarker, 1, 0);
...
if (ready == 0) {
    ...
    //Otherwise
        ...
        if (obj_mario.x < bbox_left)
        && (obj_mario.x > bbox_left-192)
            image_index = 2;
        else
            image_index = 0;
    }
